...
func main() {
	...
	containers, err := fetcher.FetchContainers()
	if err != nil {
		panic(err)
	}
	...
	for _, container := range containers {
		...
		statefulSets = append(statefulSets, builder.StatefulSet())
		persistentVolumes = append(persistentVolumes, builder.PersistentVolume())
	}
...
func main2() {
	...
	config, err := clientcmd.BuildConfigFromFlags("192.168.1.2", kubeconfig)
	if err != nil {
		panic(err)
	}
	clientset, err := kubernetes.NewForConfig(config)
